$("#signout-button").click(() => {
  ...
  $.post("/api/sign-out", () => {
    // Send POST request to /api/sign-out
...
socket.on("receive message", (message) => {
  ...
  $(".home__container__chat-view__chatlog")[0].scrollTop = $(
    ".home__container__chat-view__chatlog"
  )[0].scrollHeight; // Auto-scroll to bottom of chatlog
});

// Handling the receiving of chatlog for new connections
socket.on("receive chatlog", (messages) => {
  for (let i = messages.length - 1; i > 0; i--) {
    // Go through each message of chatlog from last (oldest) to first (newest)
...
socket.on("receive users", (users) => {
  users.forEach((user) => {
    // Go through each username of users list
...
$(".home__container__chat-view__message__send").click(() => {
  ...
  if (length > 0 && length <= 250) {
    // There is actually a message to send and it doesn't exceed character limit
